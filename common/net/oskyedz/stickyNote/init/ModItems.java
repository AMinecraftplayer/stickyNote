 ...
 * references to your items for quick access. Note there are two initialization
 * methods here, init (common, both client and server) and initClient
 * (client-side only).
 */
...
public class ModItems {
	...
	/**
	 * The common initializer. Registers items, but not models. The game will
	 * crash if we try to do anything with models on the server, as those
	 * classes don't even exist on that side!
	 * 
	...
	/**
	 * Here we call the addRecipes method for all items that have one. Should be
	 ...
	 * annotation. This causes the method to exist only on the client-side,
